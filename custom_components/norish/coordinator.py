...
class NorishListCoordinator(DataUpdateCoordinator):
    ...
    async def _fetch_trpc(self, procedure, payload=None):
        ...
        try:
            # Session hat bereits die Auth-Header!
            async with self.api_data["session"].get(url) as resp:
                if resp.status == 401:
                    _LOGGER.error(f"Norish Zugriff verweigert (401) bei {procedure}. Prüfe den API Key.")
                    return None
                if resp.status != 200:
                    _LOGGER.error(f"Norish API Fehler {resp.status} bei {procedure}")
                    return None
    ...
    async def _async_update_data(self):
        try:
            ...
            if s_data:
                # Prüfen, ob die Struktur bei API Nutzung gleich bleibt
                # Oft ist tRPC Output: [ { result: { data: { json: ... } } } ]
            ...
            if g_data:
                ...
                # Sicherheitscheck: Manchmal kommt ein Dict, manchmal Liste
